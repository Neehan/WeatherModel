...
class WeatherAutoencoderTrainer(WeatherBertTrainer):
    """
    WeatherAutoencoder trainer that inherits from WeatherBertTrainer.
    Uses the same MSE loss and training logic, just with different masking function.
    This serves as a baseline comparison against WeatherFormer.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Override masking function to match WeatherFormer for fair comparison
        self.masking_function = "weatherformer"


def weatherautoencoder_training_loop(args_dict):
    """
    WeatherAutoencoder training loop using the WeatherAutoencoderTrainer class.
    ...
    # Initialize WeatherAutoencoder model
    model = WeatherAutoencoder(
    ...
    trainer = WeatherAutoencoderTrainer(
        model=model,
